...
from engine import dispatchprio
...
class BaseBarFeed(feed.BaseFeed):
    """Base class for :class:`engine.bar.Bar` providing feeds.

    :param frequency: The bars frequency. Valid values defined in :class:`engine.bar.Frequency`.
    :param maxLen: The maximum number of values that the :class:`engine.dataseries.bards.BarDataSeries` will hold.
    ...
    # Subclasses should implement this and return a engine.bar.Bars or None if there are no bars.
    @abc.abstractmethod
    def getNextBars(self):
        """Override to return the next :class:`engine.bar.Bars` in the feed or None if there are no bars.
    ...
    def getCurrentBars(self):
        """Returns the current :class:`engine.bar.Bars`."""
        return self.__currentBars

    def getLastBar(self, instrument):
        """Returns the last :class:`engine.bar.Bar` for a given instrument, or None."""
    ...
    def getDataSeries(self, instrument=None):
        """Returns the :class:`engine.dataseries.bards.BarDataSeries` for a given instrument.

        :param instrument: Instrument identifier. If None, the default instrument is returned.
        :type instrument: string.
        :rtype: :class:`engine.dataseries.bards.BarDataSeries`.
        """
