...
from engine import stratanalyzer
...
class Returns(stratanalyzer.StrategyAnalyzer):
    """
    A :class:`engine.stratanalyzer.StrategyAnalyzer` that calculates time-weighted returns for the
    whole portfolio.
    ...
    def getReturns(self):
        """Returns a :class:`engine.dataseries.DataSeries` with the returns for each bar."""
        return self.__netReturns

    def getCumulativeReturns(self):
        """Returns a :class:`engine.dataseries.DataSeries` with the cumulative returns for each bar."""
